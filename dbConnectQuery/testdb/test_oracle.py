...
# 폴더 경로를 짧게 하기 위해서 폴더 복사 시킴
# C:\instantclient_18_5

# 1. 사용할 패키지(모듈) 임포트함
# 설치 패키지 : cx-Oracle => 임포트 시 모듈명: cx_Oracle

# 설치 실패시 :  https://visualstudio.microsoft.com/visual-cpp-build-tools/
# exe 다운받음 > 설치함
...
location = 'C:\\instantclient_18_5'
os.environ['PATH'] = location + ';' + os.environ['PATH']

# 방법 2 : 코드로 오라클 초기 설정으로 지정
# 주의사항 : 애플리케이션 전체 실행 시 딱 한번 구동되어야 함
...
# 4. 쿼리문 준비하기
query = 'select * from member'      # c##testweb.member 유저명 앞은 생략가능

# 5. 쿼리문을 실행시키키 위한 객체 준비하고, 쿼리문 실행 처리
...
# 커서가 가진 결과정보를 꺼내서 변수에 옮겨 기록 저장 처리
# result = cursor.fetchall()          #커서가 가리키는 결과 행 모두를 변수에 옮김
