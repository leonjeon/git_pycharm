...
# 파이썬에서는 예외처리 (Exception Handling)
'''
...
에러 처리 방법 :
    ...
    => 예외 상황을 처리하는 별도의 구문이 있음 - 예외 처리 구문 사용을 권장함
'''
...
def test_error():
    ...
    # 4 + new * 3     # NameError: name 'new' is not defined => 구문(문법) 에러
...
def test_input_error():
    ...
    # 해결방법 1: 입력시 조건문을 적용
    num = input('정수를 입력하세요 : ')
    if num.isdecimal():  # 정수 10진수냐? 라는 뜻임
        num = int(num)
        print(num, type(num))
    else:
...
def except_pass2():
    ...
    for item in lst:
        ...
        if str(item).isdigit():     # is...() 함수는 문자형 값에 사용하는 함수이다.
