...
# 오버로딩 : 중복 작성(정의), 이름이 같은 메소드를 클래스 안에 여러 개 만드는 것
...
class OOP:
    ...
    # + 연산자를 메서드로 오버로딩 처리
    def __add__(self, other):   # c++ : public int operator + (value) {  return this.num + value;  }
        '+ 연산자를 메서드로 오버로딩 처리'
...
ref = OOP(100)
    ...
    # 클래스 안에 연산자 오버로딩 메솓가 작성된 경우에만 객체와 값 연산이 가능함
