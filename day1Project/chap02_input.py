...
# 파이썬에서 실행시 키보드로 값 입력 받기 : Input() 함수 사용함
...
# 입력 예
# 정수 2개를 각각 입력 받아서, 사칙연산 결과 출력 처리
...
# 입력 연습
'''
신상 정보를 입력 받아, 각 변수에 저장 하시오. 변수 이름은 임의 대로 지정함
...
# 기본 출력
print(name, '은', age, '세', gender,'자 이고, 키 는', height,'cm', '몸무게는', weight,'kg 입니다.')
# format 출력
print('{}은 {}세 {}자 이고, 키는{}cm, 몸무게는 {}kg 입니다. '.format(name, age, gender, height, weight))
# f.format 출력
print(f'{name}은 {age}세 {gender}자 이고, 키는{height}cm, 몸무게는 {weight}kg 입니다. ')

#입력 연습 2
'''
키보드로 갑슬 입력받아 요구조건대로 처리하고 출력되게 코드를 작성하시오.
기본값을 가진 변수 생성 할당해 둠:
    total_point = 12500
입력 내용 : 
...
#기본 출력
print(custom_name, '고객님의 사용 금액은 ', price, '원, 발생 포인트는', int(point))
print('현재 이용하실 수 있는 누적포인트는',  int(total_point), "점입니다.")

#format() 사용
print('{} 고객님의 사용금액는 {} 원, 발생 포인트는 {} 현재 이용하실 수 있는 누적 포인트는 {} 점 입니다'.format(custom_name, price, int(point), int(total_point)))

#f.format
print(f'{custom_name} 고객님의 사용금액는 {price} 원, 발생 포인트는 {int(point)} 현재 이용하실 수 있는 누적 포인트는 {int(total_point)} 점 입니다')
