...
# 클래스(class) : 파이썬은 객체 지향형 스크립트 언어이다.

# 파이썬의 이름 작성 규칙 (식별자 조건 : Naming rule)
# 1. 대,소문자 구분한다 : name 과 Name 은 다른 이름이다.
NAME = '홍길동'
name = '이순신'
Name = '황지니'
print(NAME, name, Name)

# 2. 이름의 첫글자에 숫자 사용 못 함! : 1num (Error)
# 1num = 100
# print(1num)

# 3. 이름의 첫글자는 문자 또는 _(underscore) 만 사용할 수 있다.
_score = 100.0
print(type(_score))
print(_score)

# 4. _(underscore) 를 제외한 기호문자, 공백  사용 못 함
# $menu, all*, one num : 에러
# num& = 12     #SyntaxError
#print(num&)
...
# 6. 예약어 (프로그램 언어가 사용하기 위해서 별도로 정해놓은 단어) 는 이름으로 사용할 수 없음
#True = 1    # error
true = 1    # ok
print(true)
...
'''
여러 줄 주석
작은 또는 큰 따옴표를 앞 뒤에 3개씩 표시함
파이썬에서는 싱글쿼테이션(single quotation) or 더블쿼테이션(double quotation) 은 동일하게 취급된다.
'''
print(abs(-10))     # 10

# 파이썬에서는 변수는 반드시 값을 가져야 생성된다.
#num     # name 'num' is not defined
num = 12
print(num)

# 파이썬에서는 변수에 기록할 값의 종류(data type : 자료형) 을 정하지 않음
# 자료형 변수명 = 기록할 값 => 자료형 = 기록할 값
# 변수방에 기록된 값에 따라 자료형이 정해진다 : 동적 할당
value = 100
