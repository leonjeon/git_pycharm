...
# 키보드로 구구단의 단수를 입력 받아서, 해당 단의 구구단 출력
def print_gugudan():
    dan = int(input('단수 입력 : '))
...
# 리스트 | 투플 | 셋 안의 리스트 | 튜플 | 셋이 저장된 경우
# 첫번째 추출을 리스트 안의 아이템(리스트, 튜플, 셋) 추출임
# 꺼낸 아이템이 리스트(튜플)일 경우, 해당 아이템 안의 값들을 하나씩 추출하려면
# 이중 for 문 사용이 필요함

# 리스트 안의 아이템 안의 값의 갯수가 동일 할 경우에는 단순 for 문으로 해결 가능함
def list_in_list():
    fruit_list = [['apple', 10, 800], ['banana', 3, 2500], ['orange', 15, 500]]

    for fname, famount, fprice in fruit_list:
        print(f'{fname} 의 단가는 {fprice} 이고, 구매 수량은 {famount} 개, 구매가격은 {famount * fprice}')

# 리스트 안의 아이템의 값 갯수나 기록형태가 제각각인 경우
# 아이템 안의 각 값들을 처리하려면 이중 for 문 사용
def list_in_list2():
    nlist = [['a', 'bb', 'cba'], [10, 20], [1, 2, 3, 4, 33, 89.09]]
...
def list_in_list3():
    ...
    for index in range(len(nlist)):
        print(nlist[index])
        for aindex in range(len(nlist[index])):
            print(nlist[index][aindex])
        print('-----------------------------------------------')
