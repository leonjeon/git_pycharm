...
def sungjuk_process():
    ...
    while True:
        ...
        if no == 1:
            ...
            print(f'{sungjuk_list} 새로운 학생정보가 추가되었습니다.')

        elif no == 2:
            # 제거할 인덱스를 입력받아서 리스트에서 제거하는 코드 작성
            print('현재 저장된 데이터의 갯수는 %d개 입니다.' %len(sungjuk_list))
            # 입력받을 변수 생성(제거할 인덱스임)
            idx = int(input("제거할 아이템의 순번 입력 : "))

            if(idx >= 0 and idx < len(sungjuk_list)):
                sungjuk_list.pop(idx)   # pop는 지우는 함수!
                print(idx, '위치의 아이템이 제거되었습니다.')
                print('현재 남은 아이템의 갯수는 %d 개 입니다.' % len(sungjuk_list))
            else:
                print(f'{idx} 순번을 잘못 입력하였습니다. 확인바랍니다.')

        elif no == 3:
            # 리스트 안의 각 아이템 정보를 출력하는 코드 작성 구문
            for idx in range(len(sungjuk_list)):
                print(f'{idx} : {sungjuk_list}')
