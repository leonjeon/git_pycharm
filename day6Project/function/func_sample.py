...
def 함수명(매개변수):      => 매개변수(parameter): 0 ~ n개
    함수가 실행할 코드 구문들
    ........
    :return 값 또는 변수 또는 값, 값, 값, ... , 값 여러 개 => 받는 변수는 튜플임
...
# 아무런 기능이 없는 ( 처리할 코드가 준비중인 경우 ) 빈 함수를 만들때는 pass 사용
def func():
    pass
...
# 파이썬에서는 여러 개의 값을 리턴할 수 있음, 자동 tuple 로 처리됨
def func2(a, b):
    print(f'a: {a}, b: {b}')
    return a * 2, b * 2
...
# if __name__ == '__main__':
    ...
    # 반환값 받는 변수 = 함수명(매개변수에게 줄값(Argument), 전달인자)
    # result = add(10, 20)
    ...
    # result3 = func2(11, 22)
    # print(f'result3 : {result3}', type(result3))
...
# 함수 밖에서 지역변수 사용 못 함
# print(f'num: {num}')  =>  들여쓰기 밖 이므로 Error
...
def func_global():
    # print(f'gnum : {gnum}')     # 전역변수 선언 다음 위치에서는 어디서나 전역변수 사용 가능함
    # 파이썬에서는 변수 = 값 구문은 새로운 변수 생성(할당)임
    global  gnum     # 전역변수 사용을 선언하는 구문을 작성해야 함
    gnum = 200      # 새로 만든 지역 변수임  => 전역변수 gnum 의 값을 변경하는 구문이 되려면
    print('전역변수 gnum :', gnum)

# 함수의 매개변수(parameter)는 전달받은 값을 변경할 수 없다.
# 전달받은 값이 군집자료형일 떄는 아이템(요소)는 변경할 수 있음
def list_func(plist):
