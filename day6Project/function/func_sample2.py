...
def tmax(a, b):
    '두 개의 값을 전달받아서, 둘 중 큰값을 리턴하는 함수이다.'
...
def func_param_args():
    ...
    print(f'큰값 : {tmax("M", "m")}')
    # result2 = tmax(120)     # 에러발생 : 전달값과 매개변수 갯수가 다름
    # result2 = tmax(10, 20, 30)      # Error

#----------------------------------------------------------------------------------------------

def tmax2(a, b):        # call by value : 매개변수는 지역변수임, 지역변수가 값을 받았음
    '두 개의 값을 전달받아서, 둘 중 큰값을 리턴하는 함수이다.'
    print(f'a: {a}, b: {b}, type: {type(a)}, {type(b)}')
    max_value = 0
    if a > b:
    ...
    a = 100     # 매개변수가 받은값 변경
    b = 200     # 지역변수가 값을 변경한 것임 => 다른 함수의 변수값 영향없음
...
def func_call_value():
    ...
    print(f'큰값: {result}')
    # 함수 쪽에서 변경한 값이 호풀부의 변수에 영향을 줬는지 확인
    print(f'num1: {num1}, num2: {num2}, type: {type(num1)}, {type(num2)}')
...
def func_call_address():
    '함수쪽으로 주소 전달 테스트용 함수'
    nlist = [45, 1, 33, 12, 90, 123, 28]
    print(f'nlist : {nlist}, 주소 : {id(nlist)}')
    reslut = list_in_max(nlist)
    print(f'가장큰 값 : {reslut}')
    print(f'nlist : {nlist}')
...
#           즉, def 함수명(매개변수, 매개변수 = 기본값, 매개변수 = 기본값):       # ok
#               def 함수명(매개변수 = 기본값, 매개변수):      # error
#               def 함수명(매개변수, 매개변수 = 기본값, 매개변수):    # error

# 해당 함수 실행시 기본 매개변수에 전달값은 생략할 수 있음
# 전달값이 없으면 준비된 기본값을 사용하게 됨
# def tmain(a, b, c = 0):
# def tmain(a, b = 0, c = 0):
def tmain(a = 0, b = 0, c = 0):
...
def func_default_param():
    ...
    print('가장 작은 값 : ', tmain(12, 3, 90))
    print('가장 작은 값 : ', tmain(12, 3))
    print('가장 작은 값 : ', tmain(12))
    print('가장 작은 값 : ', tmain())

# ---------------------------------------------------------------------------------------------

# 키워드 매개변수
# 함수 사용할 때 (함수 호출시) 매개변수 = 전달값 의 형태로 사용하는 매개변수를 말함
...
def func_keyword_param():
    ...
    print('result: ', result)
    # 매개변수명을 지정해서 값 전달 처리할 수도 있음 : 키워드 매개변수 사용임
...
# 가변매개변수는 값을 0개 ~ N개임, 받을 값의 갯수가 정해지지 않았음(가변임)
# 가변매개변수의 자료형은 tuple 임
def dynamic_param(*args):
    '가변매개변수값 받은 값들 확인하는 함수'
...
def func_dynamic_param():
    ...
    dynamic_param(10)
    dynamic_param(2, 3)
    dynamic_param(4, 3, 23, 6, 5, 9, 78, 11)

# ---------------------------------------------------------------------------------------------

# 재귀함수 (재귀적 호출 함수 : Revursive Call Function)
# 함수 안에서 자신을 실행하는 함수 ( 반복문과 유사함 )
# 주의 : 반복이기 때문에 무한 루프가 되지 않도록 종료 조건을 반드시!! 기술해야 한다.
# 하지만, 파이썬은 무한 루프에 빠지면 자동으로 일정구간 반복 후에 에러 발생시킴
