...
# 모듈로 표현 : test_dict.dict_sample (디렉토리명.파일명)
# 모듈(module) : 함수들을 따로 모아서 저장해 놓은 소스 파일들의 묶음

# 사전(dict) 자료형
# 자바(Java) 의 Map 과 같은 구조로 key 와 value 를 쌍으로 저장하는 집합자료형이다.
# dict 에서 key 는 변경되지 않은 값이어야 함 (키는 지정하면 변경할 수 없음) => 키에 튜플을 사용할 수 있다.
...
# 키(key) 를 이용해서 값 변경, 조회, 추가할 수 있다. : 사전변수[키]
# 키(key) 는 변경할 수 없음 : 키에 튜플 사용할 수 있음
# 값은 자료형에 제한없음
...
# 사전1과 사전2의 동일한 키가 있을 경우에는, 사전1의 해당 키의 값이 변경됨
def test6():
    dict1 = {'name': '갤럭시', 'price': 1200000, 'tax': 0.1}
    dict2 = {'content': '최신모델입니다.', 'price': 880000}
    print('dict1 : ', dict1)
    print('dict2 : ', dict2)
...
def test7():
    ...
    # clear() 함수 : 딕셔너리 안을 비우는 함수 (저장 아이템 전체 삭제)
    dict1.clear()
    print('dict1 : ', dict1)
    ...
    # copy() 함수 :  딕셔너리 객체를 새로 만들고 아이템들을 복사한다.(deep copy)
    dict3 = dict2.copy()    #깊은 복사 (deep copy)
    print('dict3 : ', dict3, id(dict3))

    dd = dict2      # 주소복사 : 얕은복사 (shallow copy)
    print('dd :', dd, id(dd))
...
def test8():
    ...
    # 키 존재 여부 : 키 in 사전변수
    print('name 키가 존재 하느냐?', 'name' in dict1)   #True
    print('content 키가 존재 하느냐?', 'content' in dict1) #False

    # 값 존재 여부 : 값 in 사전변수.values()
    print('0.1 값이 존재 하느냐?', 0.1 in dict1.values())  #True
    print('880000 값이 존재 하느냐?',  880000 in dict1.values()) #False
