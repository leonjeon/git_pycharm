...
>> 브라우저 웹 페이지에서 검색 적용 => 검색 실행 => 잠시 대기 => 브라우저에 검색 결과가 출력
=> 파이썬에서 읽어옴 => 분석
'''
...
#   >> 브라우저 우측 상단 점3개 > 도움말 > Chrome 정보 > 버전정보 확인 : 124.0.6367.119 (최신 버전)
...
def run():
    ...
    # 네이버에 해당 웹페이지 검색 input 에 '로마여행' 이 자동 입력 됨
    ...
    try:
        ...
        # 암묵적 대기 :
        # DOM (Document Object Model : 태그 사용 계층 구조) 이 전부 다 브라우저에 로드될 때까지 (모두 읽어 들일 떄까지) 대기하게 하고
        # 먼저 로드되면 바로 태그 엘리먼트를 찾도록 진행함
        # 엘리먼트 찾을 시간(초)을 지정하면, 지정 시간동안 DOM 풀링을 지시할 수 있다.
