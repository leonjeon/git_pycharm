...
# 파이썬의 기본 파일 입출력은 텍스트 파일 입출력이다.
# 텍스트가 아닌 자료형의 파일을 다룰때는 pickle 모듈 활용함
# 바이너리(이진데이터 : binary) 형식의 파일로 저장해야 함
# 열기모드 : wb, rb, ab 로 표기해야 한다.
...
def test_os():
    ...
    print(os.listdir('.')) # '.' : 현재 디렉토리 위치를 의미함
    print(os.listdir('../'))    # '../' : 상위 디렉토리 위치를 의미함

    # rename() : 디렉토리나 파일의 이름 바꾸기함
    # os.rename('testb.txt', 'sample.txt')
    print(os.listdir('.'))
