...
# 인덱싱 표현 : 리스트변수[index]
list_3 = [1, 2, 3, 'nain', 3.14, [11, 22, 33], True, False]
print('0번째 기록된 값 조회 : ', list_3[0])
print('3번째 기록된 값 조회 : ', list_3[3])
print('5번째 기록된 값 조회 : ', list_3[5])

# 슬라이싱 : 리스트에 저장된 데이터를 부분 추출
# 표현 : 리스트변수[시작인덱스:끝인덱스:간격]
# 시작인덱스부터 끝 인덱스 -1 위치까지 추출됨
# 간격 : 생략되면 기본값 1이다.
print('0번 인덱스부터 3번 인덱스까지의 데이터를 추출한다면 : ', list_3[0:4:1])
print('0번 인덱스부터 3번 인덱스까지의 데이터를 추출한다면 : ', list_3[:4:1])

# len(리스트변수) : 리스트에 저장된 데이터 갯수 리턴
print('list_3 에 저장된 값 갯수 : ', len(list_3))
# len()을 이용해서 마지막 위치의 값 조회에 활용할 수 있음
print('list_3 에 저장된 값의 마지막 인덱스 : ', len(list_3) -1)
print('list_3 에 마지막 값 : ', list_3[len(list_3) -1])
...
list_3[1] = 'python'
print('변경 후 : ', list_3)
list_3[2] = [3.14, 2.244, 5.8]
print('변경 후 : ', list_3)

# list 자료형 특징 3 : 리스트를 다루는 함수 (메서드) 들이 제공된다.
# 리스트변수.함수명(전달인자)
# append() : 뒤에 추가
# insert() : 앨리먼트 사이에 추가
# remove() : 삭제
...
# 리스트변수.pop(index) :  해당 인덱스 위치의 값을 빼냄(제거)
num = lst.pop()
print(f'before : {lst}')
print(f'length : {len(lst)}')
print(f'num : {num}')
...
# sort() : 리스트의 저장 값들을 오름차순 정렬 처리함
# 주의 : 한 가지 종류의 값들만 저장되어 있을 때 사용할 수 있음
# lst.sort() # TypeError: '<' not supported between instances of 'list' and 'int'

lst_int = [9, 10, 55, 88, 2, 13, 44]
print(f'before sort : {lst_int}')
lst_int.sort()  # reverse=False(기본값이므로 생략) 오름차순
print(f'오름차순 after sort: {lst_int}')
lst_int.sort(reverse=True)  # 내림차순정렬
print(f'내림차순 after sort: {lst_int}')

lst_str = ['NAIN', 'nain', 'GetDrive', 'boreum']
...
print('After sort : {}'.format(lst_str))
# 한글
lst_str_kor = ['나인', '프로젝트', '겟드라이브', '전보름']
print('before sort : {}'.format(lst_str_kor))
lst_str_kor.sort()
print('After sort : {}'.format(lst_str_kor))
lst_str_kor.sort(reverse=True)
print('After sort : {}'.format(lst_str_kor))

# count() : 리스트에 저장된 똑같은 값의 갯수 조회
# 리스트변수.count(찾을값)
print(f'lst : {lst}')
# 출력 시 정수 1은 3개이다. 4개로 출력 되는 이유는 True = '1' , False = '0' 이다.
